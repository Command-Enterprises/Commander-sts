// Modded from Noctura - HUGE credits to Nebula Developer Labs for their amazing proxathon entry!
...
if (!Reflect.get(win, "location").pathname.startsWith("/games")) {
  ...
  container.innerHTML = `
    ...
    <button id="commander-close-menu">Close Menu</button>

    <input id="commander-nav-input" value="${
      location.pathname.includes("/~/aero/")
        ? new Function("return __aero$meta.href")()
        : new Function("return this.location.href")()
    }">
  ...
  container.querySelector("#commander-close-menu").onclick = function () {
    container.style.display = "none";
  };

  container.querySelector("#commander-nav-input").onkeydown = function (e) {
    if (e.key == "Enter") {
      location.pathname.includes("/~/aero/")
        ? new Function(
